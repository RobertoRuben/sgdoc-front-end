...
interface ComunicacionAreaTableProps {
  items: ComunicacionArea[];
  dataVersion: number;
  currentPage: number;
  searchTerm: string;
...
export const ComunicacionAreaTable: React.FC<ComunicacionAreaTableProps> = ({
  items,
  dataVersion,
  currentPage,
  searchTerm,
    ...
    return (
      <div className="w-full p-8 text-center">
        <p className="text-gray-500">
          No se encontraron áreas de comunicación
        </p>
      </div>
    );
  }
          ...
          <Table className="min-w-full divide-y divide-gray-200">
            ...
            <TableBody>
              {items.map((item, index) => (
                ...
                >
                  {/* You can padStart if you want a 5-digit format */}
                  ...
                  <TableCell className="px-4 py-4 whitespace-nowrap text-sm text-gray-700">
                    {item.nombreAreaOrigen ?? "—"}
                  </TableCell>

                  <TableCell className="px-4 py-4 whitespace-nowrap text-sm text-gray-700">
                    {item.nombreAreaDestino ?? "—"}
                  </TableCell>
