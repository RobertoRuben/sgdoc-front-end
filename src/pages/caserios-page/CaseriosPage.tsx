...
const CaseriosPage: React.FC = () => {
    ...
    const filteredCaserios = useMemo(() => {
        return caseriosState.data.filter(caserio => {
            return caserio.nombreCaserio.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        if (selectedCaserio?.id) {
            setCaserios({
                ...
                pagination: {
                    ...
                    totalPages: Math.ceil((caseriosState.pagination.totalItems - 1) / caseriosState.pagination.pageSize),
                },
            });
    ...
    const handleModalSubmit = async (data: CaserioDetails) => {
        if (data.id) {
            ...
            const newId = caseriosState.data.length > 0 ? Math.max(...caseriosState.data.map(x => x.id)) + 1 : 1;
    ...
    return (
        <div className="pt-0.5 pr-0.5 pb-1 pl-0.5 sm:pt-2 sm:pr-2 sm:pb-4 sm:pl-2 bg-transparent">
            <div className="flex flex-col sm:flex-row justify-between items-center mb-4 sm:mb-8 mt-4 bg-transparent">
                <h2 className="text-2xl font-bold text-gray-800 mb-2 sm:mb-0">Caserios</h2>
                <Button
                    onClick={() => {
                        setSelectedCaserio(undefined);
                        setIsModalOpen(true);
                            ...
                            <Table className="min-w-full divide-y divide-gray-200">
                                <TableHeader>
                                    <TableRow className="bg-gray-50 border-b border-gray-200">
                                        ...
                                        <TableHead className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Nombre del Caserio</TableHead>
                                        <TableHead className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Centro Poblado</TableHead>
                                ...
                                <TableBody>
                                    {currentCaserios.map((caserio, index) => (
                                        ...
                                        >
                                            <TableCell className="px-4 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{caserio.id}</TableCell>
                                            <TableCell className="px-4 py-4 whitespace-nowrap text-sm text-gray-700">{caserio.nombreCaserio}</TableCell>
                                            <TableCell className="px-4 py-4 whitespace-nowrap text-sm text-gray-700">{caserio.nombreCentroPoblado}</TableCell>
