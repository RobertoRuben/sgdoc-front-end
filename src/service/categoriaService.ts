...
export const updateCategoria = async (id: number, categoria: Omit<Categoria, "id">): Promise<Categoria | null> => {
    try {
        const payload = humps.decamelizeKeys(categoria);
        ...
        throw new Error("Error al actualizar la categoria de documento con id: " + id);
    }
};
...
export const deleteCategoria = async (id: number): Promise<boolean> => {
    try {
        ...
        throw new Error("Error al eliminar la categoria de documento con id: " + id);
    }
};
...
export const findByString = async (searchString: string): Promise<Categoria[]> => {
    try {
        ...
        if (error instanceof AxiosError) {
            if (error.response?.status === 404) {
                const notFoundError = new Error(error.response?.data?.detail || "No se encontraron resultados");
...
export const getCategoriasPaginated = async (page: number, pageSize: number): Promise<CategoriaPaginatedResponse> => {
    try {
        ...
        throw new Error("Error al obtener las categorias de documentos registradas");
    }
}